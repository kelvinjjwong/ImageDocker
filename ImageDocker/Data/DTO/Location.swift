...
class Location : NSObject {
    ...
    var coordinate:Coord? {
        didSet {
            //self.logger.log(.trace, "SET ORI COORD TO \(coordinate?.latitude) \(coordinate?.longitude)")
    ...
    var coordinateBD:Coord? {
        didSet {
            ...
            //self.logger.log(.trace, "SET BD COORD TO \(coordinateBD?.latitude) \(coordinateBD?.longitude)")
        }
    }
    ...
    public func setCoordinateWithoutConvert(coord:Coord, coordBD:Coord){
        if coord.isZero || coordBD.isZero {
            //self.logger.log(.trace, "coord zero? \(coord.isZero) coordBD zero? \(coordBD.isZero)")
        }
        self.convert = false
        self.coordinate = coord
