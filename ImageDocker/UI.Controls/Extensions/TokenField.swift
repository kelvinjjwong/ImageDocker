...
extension NSTokenField {
    ...
    fileprivate struct Keys {
        ...
        static fileprivate var tokenSeparator:UInt8 = 0
    }
    
    
    // # MARK: Public stored property
    
    public var tokenSeparator: String {
        get { return associated(key: &Keys.tokenSeparator) ?? " "}
        set {
            associate(key: &Keys.tokenSeparator, value: newValue)
        }
    }
    ...
    public var tokenStringValue: String {
        ...
        if let tokens = self.objectValue as? [ACBToken] {
            ...
            string = stringList.joined(separator: self.tokenSeparator)
        } else if let tokens = self.objectValue as? [String] {
            string = tokens.joined(separator: self.tokenSeparator)
        }
...
// default delegate, user can customize a new one
fileprivate class ACBTokenFieldController: NSObject, NSTokenFieldDelegate, NSControlTextEditingDelegate {
    ...
    @objc private func menuItemTapped(_ menuItem: NSMenuItem) {
        if let field = tokenField, let fieldEditor = field.currentEditor() {
            ...
            fieldEditor.window?.makeFirstResponder(nil)
        }
    }
